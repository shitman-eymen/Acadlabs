...
function App() {
  ...
  const formatContent = (content: string): string => {
    ...
    // Format bullet points to use numbers
    ...
    // Remove extra newlines while preserving intentional spacing
    content = content.replace(/\n{3,}/g, '\n\n');
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      for await (const { content: chunk, tokenUsage } of getGroqChatCompletion(
        ...
        const { isCode, language, content } =
          detectCodeBlock(accumulatedContent);
